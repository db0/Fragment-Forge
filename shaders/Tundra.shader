...
// Martijn Steinrucken for the Over the Moon shader
...
uniform float iTime;
uniform float direction = 1.; // change to -1. to reverse movement

const float PI=3.1415;
float S(float x,float y,float z){
	return smoothstep(x,y,z);
} 
float B(float x,float y,float z,float b){ 
	return S(x, x+b, z)*S(y+b, y, z);
}
...
uniform vec2 MOONPOS=vec2(1.3, .8);
