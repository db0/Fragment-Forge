...
void fragment() {
	...
	float color1_t_x = sin(TIME * speed_color1) / 2.0;
	...
	float color2_t_x = cos(TIME * speed_color2) / 2.0;
	float color2_t_y = sin(TIME * speed_color2) / 2.0;
	float color2_time_x = abs(cos(TIME * speed_color2));
	float color2_time_y = abs(sin(TIME * speed_color2));
	...
	float color3_time = abs(sin(TIME * speed_color3));
