...
void fragment() {
	...
	float color1_t_x = sin(iTime * speed_color1) / 2.0;
	float color1_t_y = cos(iTime * speed_color1) / 2.0;
	float color1_time_x = abs(sin(iTime * speed_color1));
	float color1_time_y = abs(cos(iTime * speed_color1));
	vec2 st = UV;
	...
	float color2_t_x = cos(iTime * speed_color2) / 2.0;
	float color2_t_y = sin(iTime * speed_color2) / 2.0;
	float color2_time_x = abs(cos(iTime * speed_color2));
	float color2_time_y = abs(sin(iTime * speed_color2));
	...
	float color3_time = abs(sin(iTime * speed_color3));
