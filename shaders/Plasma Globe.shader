...
// nimitz for the plasma globe shader
...
{   
    ...
    for( int i=0; i<VOLUMETRIC_STEPS; i++ )
    {
        ...
        sum += abs(col)*5. * (1.2-noise(lp*2.+j*13.+iTime*5.)*1.1) / (log(distance(p,orig)-2.)+.75);
    }
    return sum;
}
...
//	p.x*=iResolution.x/iResolution.y;
    ...
    mat2 mx = mm2(iTime*.4+um.x*6.);
    mat2 my = mm2(iTime*0.3+um.y*6.); 
    ro.xz *= mx;rd.xz *= mx;
    ro.xy *= my;rd.xy *= my;
...
//    #endif
    ...
    if (sph.x > 0.)
    {
        ...
        float nz = (-log(abs(flow(rf*1.2,iTime)-.01)));
        float nz2 = (-log(abs(flow(rf2*1.2,-iTime)-.01)));
