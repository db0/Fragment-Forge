...
// otaviogood for the topologica shader
...
uniform vec3 tint = vec3(0.01,0.1,1.0);
/*--------------------------------------------------------------------------------------
...
float noise2dT(vec2 uv)
{
    ...
    vec2 fl = floor(uv);
    uv = smoothv + fl;
    return textureLod(iChannel0, (uv + 0.5)/iChannelResolution0.xy, 0.0).y;	// use constant here instead?
}
float noise(vec3 uv)
{
...
//	vec2 uv = FRAGCOORD.xy/iResolution.xy * 2.0 - 1.0;// - 0.5;
	...
	vec3 finalColor = tint * density*0.2;
